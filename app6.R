...
server <- function(input, output, session) {
  ...
  questions_df <- reactive({
    ...
    df$key      <- paste0(df$category, "_$", df$value)   # lookup key
    df
  })
  ...
  # ---------- Helpers ----------
  # tile id is "Category_$Value"; convert to lookup key used in data
  ...
  # ---------- Daily Double id (5th column & $800 must exist) ----------
  dd_id <- reactive({
    cats <- categories(); vals <- values()
    if (length(cats) >= 5 && any(gsub("\\$","", vals) == "800")) {
      paste0(cats[5], "_$800")
    } else NA_character_
  })
